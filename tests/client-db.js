...
describe('db.readOneByKey', function () {
...
describe('db.readwith unique indexes', function () {
    ...
    before(function(done) {
        ...
        db = new DB(databaseName2, databaseVersion2, tablesUnique);
        hash.push(db.save('presidents', {name: 'Barack', lastName: 'Obama', 'birth': 1961}));
        hash.push(db.save('presidents', {name: 'John F.', lastName: 'Kennedy', 'birth': 1917}));
        hash.push(db.save('presidents', {name: 'John F.', lastName: 'Kennedy another', 'birth': 1917}));
        hash.push(db.save('presidents', {name: 'Bill', lastName: 'Clinton', 'birth': 1946}));
    ...
    after(function(done) {
        var db = new DB(databaseName2, databaseVersion2, tablesUnique);
        db.deleteDatabase().finally(function() {
            done();
        });
    });
...
describe('db.readwith unique indexes force overwrite', function () {
    ...
    before(function(done) {
        ...
        prevErrorHandler = window.onerror;
        window.onerror = function(){return true;};
        db = new DB(databaseName3, databaseVersion3, tablesUnique);
        hash.push(db.save('presidents', {name: 'Barack', lastName: 'Obama', 'birth': 1961}));
        hash.push(db.save('presidents', {name: 'John F.', lastName: 'Kennedy', 'birth': 1917}));
    ...
    after(function(done) {
        var db = new DB(databaseName3, databaseVersion3, tablesUnique);
        db.deleteDatabase().finally(function() {
            done();
        });
    });
